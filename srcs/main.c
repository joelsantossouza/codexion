...
void	clean(t_coder coders[], t_dongle dongles[])
{
	...
	if (coders)
		destroy_coders(config->num_coders, coders);
	if (dongles)
		destroy_dongles(config->num_coders, dongles);
}
...
int	handle_special_cases(void)
{
	...
	if (config->num_coders == 0 || config->num_compiles_required == 0)
		return (1);
...
int	main(int argc, char **argv)
{
	...
	if (handle_special_cases() != 0)
		return (0);
	if (init_dongles(config.num_coders, dongles) != 0)
		return (err_log(ERR_DONGLES_INIT), 1);
	if (init_coders(config.num_coders, coders, dongles) != 0)
	...
	init_coder_log();
	if (start_coders(config.num_coders, coders, START_PATTERN) != 0)
		return (clean(coders, dongles), err_log(ERR_THREAD_INIT), 1);
